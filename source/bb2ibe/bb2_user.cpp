...
namespace BB2
{
  ...
  void canonical(std::vector<unsigned char> &s, const Fp12 &v, int o)
  {
    ...
    if (o != -1)
    {
      for (int i = 0; i < n; i++)
      {
        ...
        mpz_class a_mpz(v2[i].a.getStr());
        mpz_class b_mpz(v2[i].a.getStr());
        mpzUtil::mpzToBytes(a, a_mpz, l, o);
        mpzUtil::mpzToBytes(b, b_mpz, l, o);
  ...
  {
    if (n < msg.size())
      throw std::invalid_argument("BB2::User::encrypt: n less than sizeof msg");
    ...
    {
      pad = n - msg.size();
      if (pad > UCHAR_MAX || (pad == 0 && n > UCHAR_MAX))
        throw std::runtime_error("BB2::User::encrypt: too large padding");
      if (pad == 0)
      {
        pad = n;
        n *= 2;
  ...
  {
    if (!belong)
      throw std::runtime_error("BB2::User::encrypt: user don't belong to any KGC");
  ...
  {
    if (!belong)
      throw std::runtime_error("BB2::User::decrypt: user don't have key");
